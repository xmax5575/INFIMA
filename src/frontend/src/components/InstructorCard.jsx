...
export default function InstructorCard({
  ...
  const priceLabel = user?.price_label ?? (user?.price_eur != null ? `${user.price_eur}€` : "—");
      ...
      <div className="flex gap-0.5">
        {Array.from({ length: maxStars }).map((_, i) => (
          <Star
            key={i}
            className={`h-4 w-4 ${
              i < rounded ? "text-[#3674B5] fill-[#3674B5]" : "text-[#3674B5]/30"
            }`}
          />
        ))}
      </div>
  ...
  const formatSlot = (slot) => {
    ...
    const d = slot.date || "";
    const place = slot.format === "Online" ? "Online" : slot.location?.trim() ? slot.location : "Uživo";
    return `${d} ${t} • ${place}`;
  };
    ...
    <div className="w-full">
      <div className="relative mx-auto w-full max-w-5xl rounded-3xl bg-[#D1F8EF] p-5 sm:p-7 shadow-sm">
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full">
            <div className="h-[210px] w-full sm:max-w-[220px] overflow-hidden rounded-2xl bg-[#808080] shrink-0">
              <img src={avatarUrl || defaultAvatar} alt="Avatar" className="h-full w-full object-cover" />
            </div>
            <div className="h-[210px] flex-1 overflow-hidden rounded-2xl shadow-inner">
              <GoogleMapEmbed location={user.location} className="h-full w-full" />
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-12">
          ...
          {/* LIJEVO: Područja, Video i Recenzije */}
          <div className="lg:col-span-5 flex flex-col gap-5">
            ...
            {/* 2. VIDEO - SADA JE IZNAD RECENZIJA */}
            {videoUrl && (
            ...
            {/* 3. Recenzije - SADA SU ISPOD VIDEA */}
            <div className="rounded-2xl bg-white h-[280px] flex flex-col overflow-hidden border border-[#3674B5]/10 shadow-sm">
              <div className="p-5 border-b border-[#3674B5]/10 flex items-center gap-3 bg-white sticky top-0 z-10">
                <div className="text-[#3674B5] text-3xl font-semibold">{rating}</div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3">
                {loadingReviews ? (
                  <p className="text-sm text-[#3674B5]/60 italic text-center">Učitavanje...</p>
                ) : reviews.length === 0 ? (
                  <p className="text-sm text-[#3674B5]/60 italic text-center">Nema recenzija</p>
                ) : (
                  reviews.map((r, i) => (
                    <div key={i} className="p-3 rounded-xl border border-[#3674B5]/10 bg-slate-50/50">
                      <div className="flex justify-between items-center mb-1">
                        {renderStars(r.rating)}
                        <span className="text-xs font-bold text-[#3674B5]">{r.student_first_name || "Korisnik"}</span>
                      </div>
                      <p className="text-sm text-[#3674B5]/80 leading-snug">{r.description}</p>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="lg:col-span-7 flex flex-col">
            <div className="rounded-2xl bg-[#3674B5] p-5 sm:p-6 text-[#D1F8EF] h-full flex flex-col shadow-md">
              <h2 className="text-lg font-semibold text-center mb-4">Kalendar instruktora</h2>
              ...
              <div className="mt-auto">
                <p className="text-xs font-bold uppercase tracking-wider mb-2 opacity-70">Dostupni termini:</p>
                <div className="flex flex-wrap gap-2">
                  {lessons.length > 0 ? (
                    lessons.slice(0, 6).map((slot) => (
                      <span key={slot.lesson_id} className="rounded-full bg-white/15 px-3 py-1 text-xs border border-white/10">
                        {formatSlot(slot)}
                      </span>
                    ))
                  ) : (
                    <span className="text-xs opacity-50 italic">Nema upisanih termina</span>
                  )}
                </div>
              </div>
            </div>
        ...
        {canEdit && (
          <div className="mt-6">
            <Link to={editTo} className="block w-full text-center rounded-xl bg-[#215993] px-4 py-3 text-[#D1F8EF] font-semibold hover:brightness-110 shadow-sm transition-all">
              Uredi profil
            </Link>
          </div>
        )}
